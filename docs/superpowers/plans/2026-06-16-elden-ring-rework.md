...
describe("LEVELS config", () => {
  ...
  it("sin vocabulario Dark Souls en nombres visibles", () => {
    const text = LEVELS.map((l) => `${l.name} ${l.runeName}`)
      .join(" ")
      .toLowerCase();
...
export const LEVELS = [
  {
    ...
    tagline:
      "Una capilla en ruinas donde el agua aún recuerda el rito del ungimiento.",
    description:
  ...
  },
  {
    ...
    difficulty: "Prueba",
    tagline:
      "La forja de los gigantes aún arde, alimentada por llamas que recuerdan.",
    description:
    ...
    rewardFlavor: "Dos sacos de granos oscuros, tostados al calor de la forja.",
  ...
  },
  {
    ...
    tagline:
      "Un resplandor dorado llama desde el trono áureo, y exige tu instinto.",
    description:
...
export const TOTAL_RUNES = LEVELS.reduce(
  (sum, level) => sum + level.runeCount,
  0,
);
```
...
describe("progressLogic", () => {
  ...
  it("un nivel se desbloquea solo si el anterior está completo", () => {
    ...
    expect(
      isLevelUnlocked("golden-lord", ["vanilla-shrine", "ember-forge"]),
    ).toBe(true);
    expect(
      isLevelUnlocked("grinding-hollow", [
        "vanilla-shrine",
        "ember-forge",
        "golden-lord",
      ]),
  ...
  it("nextLevelId devuelve el primer nivel no completado por orden", () => {
    ...
    expect(nextLevelId(["vanilla-shrine", "ember-forge", "golden-lord"])).toBe(
      "grinding-hollow",
    );
  });
    ...
    <span className="relative inline-flex items-center justify-center">
      <span
        ...
        style={
          locked
            ? undefined
            : { textShadow: "0 0 16px rgba(200, 168, 78, 0.6)" }
        }
        role="img"
        aria-label={label}>
        {rune}
      ...
      {!locked && count > 1 && (
        <span className="ml-1 self-start font-heading text-base text-bronze">
          ×{count}
        </span>
      )}
    </span>
...
export default function ChestReveal({ level, animate }) {
  ...
  return (
    <div
      ...
      <p className="font-heading text-xs uppercase tracking-[0.3em] text-bronze">
        {opened
          ? "El cofre se abre con un crujido..."
          : "Un cofre espera, atado con hierro viejo"}
      </p>
...
const FLAVOR_FAIL =
  "El enemigo golpea con precisión... Reponte, Tarnished, e inténtalo de nuevo.";
...
const FLAVOR_VICTORY =
  "El guardián cae, disolviéndose en motas de luz dorada...";
...
export default function VanillaShrine({ onComplete }) {
  ...
  const handleParry = () => {
    ...
    const inZone =
      indicatorPos >= ZONE_START && indicatorPos <= ZONE_START + zoneWidth;
    ...
    if (inZone) {
      ...
      if (nextStreak >= PARRIES_NEEDED) {
        ...
        addTimeout(
          () => setGuardianState((s) => (s === "hit" ? "idle" : s)),
          FEEDBACK_MS,
        );
    ...
    return (
      <div className="fade-in flex flex-col items-center gap-5 py-4 text-center">
        ...
        <p className="font-body text-sm leading-relaxed text-bone/80">
          Una runa ha sido tuya. Pero queda una segunda ofrenda en el altar, más
          veloz y esquiva. Demuestra de nuevo tu temple, Tarnished.
        </p>
        <button
          type="button"
          onClick={beginSecondOffering}
  ...
  return (
    <div className="relative flex flex-col items-center gap-5 text-center">
      ...
      <p className="min-h-[3.5rem] font-body text-sm leading-relaxed text-bone/80">
        {message}
      </p>
...
const MSG_VICTORY =
  "Las llamas se calman, satisfechas: la forja entrega su regalo...";
...
export default function EmberForge({ onComplete }) {
  ...
  if (phase === "attempt-clear") {
    return (
      <div className="fade-in flex flex-col items-center gap-5 py-4 text-center">
        ...
        <p className="font-body text-sm leading-relaxed text-bone/80">
          Una runa arde ya en tu mano. Pero la forja guarda un segundo himno,
          más veloz que el primero. Escúchalo bien, Tarnished.
        </p>
        <button
          type="button"
          onClick={beginSecondAttempt}
          className="souls-button w-full py-4 text-lg">
  ...
  return (
    <div className="flex flex-col items-center gap-5 text-center">
      <p className="font-heading text-xs uppercase tracking-[0.3em] text-bronze">
        Himno {attempt + 1}/{TOTAL_ATTEMPTS} · Brasa{" "}
      ...
      <div className="ember-grid">
        ...
        <div className="ember-center">
          ...
          {showSpark && (
            <div
              className="ember-spark-container"
              key={`${attempt}-${round}-${phase}`}>
      ...
      <p className="min-h-[3rem] font-body text-sm leading-relaxed text-bone/80">
        {message}
      </p>
    </div>
  );
...
describe("goldenLordSpeed", () => {
  ...
  it("dentro de una ronda, los ataques se aceleran con el índice", () => {
    expect(fallDurationFor(0, ATTACKS_PER_ROUND - 1)).toBeLessThan(
      fallDurationFor(0, 0),
    );
  });
});
...
const factorFor = (round) =>
  ROUND_SPEED[Math.min(round, ROUND_SPEED.length - 1)];

export const fallDurationFor = (round, index) =>
  lerp(FALL_MS_START, FALL_MS_END, index / (ATTACKS_PER_ROUND - 1)) *
  factorFor(round);

export const spawnIntervalFor = (round, index) =>
  lerp(SPAWN_MS_START, SPAWN_MS_END, index / (ATTACKS_PER_ROUND - 1)) *
  factorFor(round);
```
...
const MSG_VICTORY =
  "El Golden Lord se arrodilla ante ti, radiante y humillado al fin...";
...
export default function GoldenLord({ onComplete }) {
  ...
  useEffect(() => {
    ...
    const loop = (now) => {
      ...
      if (!pausedRef.current) {
        ...
        if (
          spawnedCountRef.current < ATTACKS_PER_ROUND &&
          elapsed >= nextSpawnRef.current
        ) {
          ...
          nextSpawnRef.current =
            elapsed + spawnIntervalFor(currentRound, index);
          setShowHint(false);
        }
        ...
        for (const attack of attacksRef.current) {
          const progress =
            (elapsed - attack.spawnElapsed) / attack.fallDuration;
          if (progress >= 1) {
            resolvedCount += 1;
            const inPlayerLane = attack.lane === playerLaneRef.current;
            if (inPlayerLane && elapsed >= invulnUntilRef.current)
              tookHit = true;
          } else {
        ...
        if (resolvedCount > 0) {
          survivedRef.current = Math.min(
            survivedRef.current + resolvedCount,
            ATTACKS_PER_ROUND,
          );
        ...
        if (!nextPhase && survivedRef.current >= ATTACKS_PER_ROUND) {
          nextPhase =
            currentRound >= TOTAL_ROUNDS - 1 ? "victory" : "round-clear";
        }
  ...
  const handleDodgeRight = useCallback(
    () => handleInput("right"),
    [handleInput],
  );
  ...
  if (phase === "round-clear") {
    return (
      <div className="fade-in flex flex-col items-center gap-5 py-6 text-center">
        ...
        <p className="font-body text-sm leading-relaxed text-bone/80">
          Una pieza del trono es tuya. El Señor se enfurece y su danza se
          acelera. Prepárate, Tarnished.
        </p>
        <button
          type="button"
  ...
  if (phase === "victory") {
    return (
      <div className="flex flex-col items-center gap-6 py-4 text-center">
        <span
          className="lord-victory-figure"
          role="img"
          aria-label="El Golden Lord se arrodilla">
          👑
        </span>
        <p className="font-display text-2xl shimmer-text">
          EL GOLDEN LORD SE ARRODILLA
        </p>
        <p className="font-body text-sm leading-relaxed text-bone/80">
          {MSG_VICTORY}
        </p>
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="lord-arena golden-glow">
        ...
        <div className="lord-hud">
          <div
            className="lord-lives"
            aria-label={`${lives} de ${STARTING_LIVES} vidas restantes`}>
        ...
        <div
          className={`lord-flash ${hitFlash ? "is-active" : ""}`}
          aria-hidden="true"
        />
...
describe("grindingCipher", () => {
  ...
  it("hay runas suficientes para todas las letras únicas", () => {
    expect(RUNE_POOL.length).toBeGreaterThanOrEqual(
      uniqueLettersOf(MESSAGE).length,
    );
  });
...
const MSG_FAIL =
  "El texto se resiste. Las runas vuelven a quedar quietas y silenciosas.";
const MSG_VICTORY =
  "La piedra se enciende con fuego escarlata, y la verdad queda al descubierto...";
...
export default function GrindingHollow({ onComplete }) {
  ...
  const healthPercent = Math.max(
    0,
    100 - Math.round((correctCount / TOTAL_LETTERS) * 100),
  );
  ...
  if (phase === "intro") {
    return (
      <CinematicIntro pages={INTRO_PAGES} onDone={() => setPhase("puzzle")} />
    );
  }
  ...
  return (
    <div className="boss-frame relative flex flex-col gap-5 p-4 text-center">
      ...
      <p className="font-body text-sm italic leading-relaxed text-bone/70">
        En el santuario marchito del Haligtree, el nombre de la Cuchilla de
        Miquella fue grabado en runas antiguas. Solo los dignos sabrán leer lo
        que está escrito en la piedra.
      </p>
      ...
      {selectedRune && (
        <div className="souls-panel fade-in flex flex-col gap-3 p-4">
          <p className="font-heading text-xs uppercase tracking-[0.25em] text-bronze">
            Asigna una letra a{" "}
            <span className="text-xl text-gold">{selectedRune}</span>
          </p>
...
<p className="max-w-xs font-body text-sm leading-relaxed text-bone/80">
  La ceniza cae sobre una tierra olvidada. Tres pruebas esperan... y más allá de
  ellas, una cuarta: dorada, despiadada, reservada para quienes perseveren.
</p>
```
...
```jsx
<p className="max-w-xs font-body text-sm leading-relaxed text-bone/80">
  La gracia ilumina las Tierras Intermedias. Cuatro pruebas te aguardan en el
  camino, y al final de todas, la Cuchilla de Miquella. Reclama sus runas y
  preséntalas ante la Doncella.
</p>
```
...
<meta
  name="description"
  content="The Tarnished's Birthday — una prueba al estilo Souls repleta de desafíos y runas." />
```
...
<meta
  name="description"
  content="The Tarnished's Birthday — atrévete a enfrentar tu destino." />
```
